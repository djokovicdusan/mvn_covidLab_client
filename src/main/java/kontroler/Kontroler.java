...
public class Kontroler {
    ...
    public List<Pacijent> dajSvePacijente() throws Exception {
        ...
        kz.setOperacija(Operacije.VRATI_SVE_PACIJENTE);
        out.writeObject(kz);
    ...
    public List<Laborant> dajSveLaborante() throws Exception {
        KlijentskiZahtev kz = new KlijentskiZahtev();
        kz.setOperacija(Operacije.VRATI_SVE_LABORANTE);
        out.writeObject(kz);
        ServerskiOdgovor so = (ServerskiOdgovor) in.readObject();
        if (so.getOperacija() == Operacije.DISKONEKTUJ_ADMINISTRATORA) {
            exitApp();
    ...
    public Administrator ulogujAdministratora(String username, String password) throws Exception {
        ...
        kz.setData(new Administrator(username, password, "", ""));
        out.writeObject(kz);
        ServerskiOdgovor so = (ServerskiOdgovor) in.readObject();
        if (so.getOperacija() == Operacije.DISKONEKTUJ_ADMINISTRATORA) {
            exitApp();
...
//   public void sacuvajRezultat(Rezultat rezultat) throws Exception {
//        KlijentskiZahtev kz = new KlijentskiZahtev();
//        kz.setOperacija(Operacije.SACUVAJ_REZULTAT);
//        kz.setData(rezultat);
//        out.writeObject(kz);
...
//    }
    ...
    public List<Test> dajSveTestove() throws Exception {
        KlijentskiZahtev kz = new KlijentskiZahtev();
        kz.setOperacija(Operacije.VRATI_SVE_TESTOVE);
        out.writeObject(kz);
        ServerskiOdgovor so = (ServerskiOdgovor) in.readObject();
        if (so.getOperacija() == Operacije.DISKONEKTUJ_ADMINISTRATORA) {
            exitApp();
    ...
    public List<TerminTestiranja> dajSveTermineTestiranja() throws Exception {
        KlijentskiZahtev kz = new KlijentskiZahtev();
        kz.setOperacija(Operacije.VRATI_SVE_TERMINE_TESTIRANJA);
        out.writeObject(kz);
        ServerskiOdgovor so = (ServerskiOdgovor) in.readObject();
        if (so.getOperacija() == Operacije.DISKONEKTUJ_ADMINISTRATORA) {
            exitApp();
    ...
    public List<Rezultat> dajSveRezultate() throws Exception {
        KlijentskiZahtev kz = new KlijentskiZahtev();
        kz.setOperacija(Operacije.VRATI_SVE_REZULTATE);
        out.writeObject(kz);
        ServerskiOdgovor so = (ServerskiOdgovor) in.readObject();
        if (so.getOperacija() == Operacije.DISKONEKTUJ_ADMINISTRATORA) {
            exitApp();
    ...
    private void exitApp() {
        ...
        JOptionPane.showMessageDialog(null, "Izvinjavamo se server je prestao sa radom, program će se ugasiti, pokušajte ponovo kasnije");
        System.exit(0);
    }
